...
def inverse_kinematics(object_pos):
    ...
    theta5 = 0  # Assuming no rotation in 5th axis
...
object_positions = [(x, y, z) for x in np.arange(0, 22.1, 0.1) for y in np.arange(0, 22.1, 0.1) for z in np.arange(0, 2.5, 0.1)]
...
for obj_pos in object_positions:
    ...
    if thetas is not None:
        results.append([*obj_pos, *thetas])

# Create DataFrame
columns = ['x', 'y', 'z', 'theta1 (deg)', 'theta2 (deg)', 'theta3 (deg)', 'theta4 (deg)', 'theta5 (deg)']
df = pd.DataFrame(results, columns=columns)

# Save to Excel
output_path = r'C:\Users\TUAN\Capstone Project\workspace\vertical_grasp_5.xlsx'
